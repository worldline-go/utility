...
func TestClient_RequestWithURL(t *testing.T) {
	httpServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// check request method
		if r.Method != http.MethodPost {
			w.WriteHeader(http.StatusBadRequest)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			URL, err := url.JoinPath(httpServer.URL, "/api/v1/test")
			if err != nil {
				t.Errorf("url.JoinPath() error = %v", err)
				return
			}

			if err := httpxClient.RequestWithURL(tt.args.ctx, tt.args.method, URL, tt.args.body, tt.args.header, tt.args.fn); (err != nil) != tt.wantErr {
				t.Errorf("Client.Request() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
